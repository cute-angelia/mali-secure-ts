"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
// var Secure = require('./secure.js')
var secure_1 = require("./secure");
var appid = "appidxxxx";
var cid = 1;
var appSecret = "secret";
var version = "1.0.1";
var s = new secure_1.Secure(appid, cid, appSecret, version, "", "");
var uri1 = s.getSign('https://mp.weixin.qq.com/wxamp/devprofile/get_profile?token=1515154505&lang=zh_CN');
console.log(uri1);
// let z = {"code":0,"crypto":"mDeTotIpUkpOYLozkRYdjbymLRiknTHB","data":"","msg":"成功!"}
// console.log(s.decrypt(z))
